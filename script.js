...
function clearSignalPlan(){
  set('signalAction','WAIT');
  set('signalEntry','—');
  set('signalStop','—');
  set('signalTarget','—');
  set('signalInvalidation','—');
  set('signalWaiting','Awaiting confirmed setup');
}
function renderSignalPlan(a){
  const x=a||{};
  const action=String(x.action_state||'WAIT').toUpperCase();
  set('signalAction',action);
  const entry=x.entry_price??x.reference_level;
  const stop=x.stop_loss??x.invalidation_level;
  const target=x.take_profit_1??(Array.isArray(x.target_levels)?x.target_levels[0]:undefined);
  const actionable=['BUY','SELL','BUY AREA','SELL AREA'].includes(action);
  set('signalEntry',actionable?fmt(entry):'—');
  set('signalStop',actionable?fmt(stop):'—');
  set('signalTarget',actionable?fmt(target):'—');
  set('signalInvalidation',actionable?fmt(x.invalidation_level):'—');
  set('signalWaiting',x.waiting_for||(!actionable?'Awaiting confirmed setup':'—'));
}
...
function mountTradingView(){
 ...
 script.text=JSON.stringify({autosize:true,width:"100%",height:"100%",symbol:chartSymbol(),interval:tvInterval(),timezone:'Africa/Johannesburg',theme:'light',style:'1',locale:'en',allow_symbol_change:false,calendar:false,hide_top_toolbar:false,hide_legend:false,save_image:false,hide_side_toolbar:false,support_host:'https://www.tradingview.com'});
...
async function runAIAnalysis(macro){const payload={asset:state.asset,timeframe:state.timeframe,depth:$('depth').value,markets:state.markets,news:macro?.news||[],calendar:macro?.calendar||[],chartSource:'TradingView embedded live chart; analysis is based on synchronized OHLC market data for the same instrument and timeframe.'};try{const r=await fetch('/api/analyze',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}),j=await r.json().catch(()=>({}));if(!r.ok)throw new Error(j.error||'AI request failed');state.ai=j;const a=j.analysis;if(!a){clearSignalPlan();set('apiMode','ENGINE — LOCAL TECHNICAL');return}set('actionState',a.action_state||'WAIT');set('setupState',a.action_state||'WAIT');set('lastCandle',a.candle_reading||$('lastCandle').textContent);set('entryPlan',[a.entry_plan,a.scenario].filter(Boolean).join(' — ')||'—');set('actionReason',a.action_reason||a.direction_reason||'—');renderSignalPlan(a);set('bias',a.higher_timeframe_bias||timeframeBias());set('marketState',a.market_state||'WAIT');set('quality',Number.isFinite(Number(a.analysis_quality))?Number(a.analysis_quality).toFixed(0)+' / 100':$('quality').textContent);$('qualityBar').style.width=Math.min(100,Math.max(0,Number(a.analysis_quality)||0))+'%';set('biasReason',a.direction_reason||'');set('apiMode','ENGINE — OPENAI + TECHNICAL + NEWS');const rr=Array.isArray(a.reasoning)?a.reasoning:[];if(rr.length)$('reasoning').innerHTML=rr.map(x=>'<li>'+esc(x)+'</li>').join('');if(a.news_assessment||a.news_events?.length){set('macroRisk',a.news_events?.length?'REVIEW':'LOW');$('macro').innerHTML='<p>'+esc(a.news_assessment||'')+'</p>'+(a.news_events||[]).map(x=>'<div><small>'+esc(x)+'</small></div>').join('')}}catch(e){state.ai={available:false,error:e.message};clearSignalPlan();set('apiMode','ENGINE — LOCAL TECHNICAL')}}
async function analyzeMarket(){const btn=$('analyzeMarket');btn.disabled=true;btn.innerHTML='Analyzing…';clearSignalPlan();status(false,'LOADING');state.timeframe=$('timeframe').value;mountTradingView();try{const tfs=['4h','1h','15m','5m'],loaded=await Promise.allSettled(tfs.map(loadMarket));state.markets={};loaded.forEach((r,i)=>{if(r.status==='fulfilled')state.markets[tfs[i]]=r.value});const m=state.markets[state.timeframe]||state.markets['15m']||state.markets['5m']||state.markets['1h']||state.markets['4h'];if(!m)throw new Error('No live market-data endpoint is reachable. Deploy the API on Vercel for server-side market access.');state.current=m;renderTimeframes();renderMain(m);const macro=await loadMacro();await runAIAnalysis(macro);resolveOpenSignals(m);if(state.ai?.analysis)recordSignal(state.ai.analysis);if(state.activePage==='performance')renderPerformance();status(true,state.ai?.analysis?'LIVE + AI':'LIVE + LOCAL')}catch(e){status(false,'DATA OFFLINE');set('marketState','DATA UNAVAILABLE');set('bias','NEUTRAL');set('biasReason',e.message);set('dataSource','DATA OFFLINE');clearSignalPlan()}finally{btn.disabled=false;btn.innerHTML='Analyze Market <span>↗</span>'}}
document.addEventListener('DOMContentLoaded',()=>{
 state.timeframe=$('timeframe').value;
 document.querySelectorAll('.asset-tab').forEach(tab=>tab.addEventListener('click',()=>{state.asset=tab.dataset.chartAsset;mountTradingView()}));
 ...
 mountTradingView();
 analyzeMarket();
 setInterval(async()=>{try{const m=await loadMarket(state.timeframe);state.markets[state.timeframe]=m;state.current=m;resolveOpenSignals(m);if(state.activePage==='performance')renderPerformance();}catch(e){/* keep last verified state */}},15000);
 document.querySelectorAll('.nav-btn').forEach(b=>b.addEventListener('click',()=>showPage(b.dataset.page)));
 const clear=$('clearPerformance');if(clear)clear.addEventListener('click',()=>{if(confirm('Clear this browser performance history?')){localStorage.removeItem(SIGNAL_KEY);renderPerformance()}});
 setInterval(()=>{if(state.activePage==='performance')renderPerformance()},5000);
});
...
function recordSignal(a){
  ...
  const entry=Number(a.entry_price ?? a.reference_level),sl=Number(a.stop_loss ?? a.invalidation_level),tp=Number(a.take_profit_1 ?? a.target_levels?.[0]);
